...
6. `StaticPortabilityAnalyzer` проверяет forward/reverse plugin dependencies, local resources,
   traversal, symlinks, plugin-owned symbols и external requirements.
...
Для blob URL discovery scope автоматически становится parent directory выбранного файла. Явный
`--subpath` переопределяет scope из URL. Для branch/tag с `/` или другого неоднозначного GitHub
route передавайте `--ref`; resolver сверяет branch/tag names через `git ls-remote`.
...
4. Copy повторно проверяет source inode/type/size/hash/link count и safe symlink chain. Regular
   files синхронизируются через `fsync` строго, directories — там, где filesystem это поддерживает.
