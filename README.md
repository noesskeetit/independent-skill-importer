...
Transport/contract failure получает не более одной повторной FM-попытки в рамках общего
`max_fm_reviews`; после двух невалидных ответов candidate остаётся `ambiguous`. Номер evidence
может быть исправлен только на одну соседнюю строку, если найдено ровно одно совпадение того же
значения в том же файле; дальние, неоднозначные и отсутствующие цитаты отклоняются.

Перед отправкой repository data ограничиваются context budget, sensitive files исключаются, а
credential-like values редактируются. Текстовые файлы передаются как `files[].lines[]` с явными
номерами строк. Ответ привязан к SHA-256 exact canonical envelope; evidence перепроверяется по
строкам и hash immutable inventory. Повышение `ambiguous -> portable` возможно
...
| `fm_timeout_seconds` | 60 s | Один Cloud.ru FM HTTP request. |
